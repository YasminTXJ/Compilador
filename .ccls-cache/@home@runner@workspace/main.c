...
void *verificaSeTemMemoriaDisponivelEFazOMalloc(size_t size) {
    ...
    if (total_memory_used > max_memory_used) max_memory_used = total_memory_used;
    return ptr;
}
...
void LiberaMallocELiberaMemoria(void *ptr, size_t size) {
    if (ptr) {
        free(ptr);
        if (total_memory_used >= size) total_memory_used -= size;
    }
    if (ptr == NULL) {
        if (total_memory_used >= size) total_memory_used -= size;
    }
...
int VerificaSeNomeDeVarOuFuncEhValido(const char *token) {
    for (int i = 0; token[i] != '\0'; i++)
        if (!isalpha(token[i]) && !isdigit(token[i])) return 0;
    return 1;
}

int ehVariavel(const char *token) {
    if (token[0] == '!' && islower(token[1])) {
        if (strlen(token) > 2) return VerificaSeNomeDeVarOuFuncEhValido(token + 2);
        else return 1;
    }
    return 0;
}
...
Simbolo *adicionarSimbolo(Simbolo *inicio, const char *tipo, const char *nome,
  ...
  // Verifica se já existe
    ...
    if (!novo) { printf("Memória insuficiente!\n"); exit(1); }
...
Simbolo *atribuirValor(Simbolo *inicio, const char *nome, const char *valor, const char *funcao) {
  // Verifica se já existe
    Simbolo *existe = buscarSimbolo(inicio, nome, funcao);
    ...
    if (existe != NULL) {
      //Verificar se o tipo do valor é compativel com o tipo da variavel
       ...
       printf("Impossivél atribuir valor a uma variavél não declarada.\n", nome, valor);
    }
}
...
void addTokenToList(const char *token, int linha) {
    ...
    TokenNode *novo = (TokenNode *)verificaSeTemMemoriaDisponivelEFazOMalloc(sizeof(TokenNode));
    novo->token = safe_strdup(token);
    novo->linha = linha;
    novo->prox = NULL;

    if (!token_list) token_list = token_list_tail = novo;
    else { token_list_tail->prox = novo; token_list_tail = novo; }
    tokenCount++;
}
...
void tokenizeLine(const char *line, int num_linha) {
    ...
    while (i < len) {
        ...
        if (!dentroDeString && !verificaAsciiValido(c)) { printf("[LINHA %d] ERRO: %c\n", num_linha, c); i++; break; }

        switch(c) {
            case ' ': case '\t': case '\n': case '\r':
                token[j]='\0'; addTokenToList(token, num_linha); j=0; i++; break;
            case '"':
                j=0; token[j++]=line[i++];
                while(i<len && line[i]!='"') token[j++]=line[i++];
                if(i<len && line[i]=='"') token[j++]=line[i++];
                token[j]='\0'; addTokenToList(token, num_linha); j=0;
                break;
            case '!':
                token[j]='\0'; addTokenToList(token, num_linha); j=0; token[j++]=c; i++;
                while(i<len && isalnum(line[i])) token[j++]=line[i++];
                token[j]='\0'; addTokenToList(token, num_linha); j=0; break;
            case '=': case '<': case '>': case '&': case '|':
                token[j]='\0'; addTokenToList(token, num_linha); j=0;
                if ((c=='=' && line[i+1]=='=') || (c=='<'&&(line[i+1]=='='||line[i+1]=='>')) || 
                    (c=='>'&&line[i+1]=='=') || (c=='&'&&line[i+1]=='&') || (c=='|'&&line[i+1]=='|')) {
                    char op[3]={c,line[i+1],'\0'}; addTokenToList(op,num_linha); i+=2;
                } else { char op[2]={c,'\0'}; addTokenToList(op,num_linha); i++; } break;
            case '+': case '-': case '*': case '/': case '^':
            case '(': case ')': case '{': case '}': case '[': case ']': case ';': case ',':
                token[j]='\0'; addTokenToList(token,num_linha); j=0; char s[2]={c,'\0'}; addTokenToList(s,num_linha); i++; break;
            default: token[j++]=c; i++; break;
        }
    }
    token[j]='\0'; addTokenToList(token, num_linha);
}
...
TokenNode *tokenizeFile(const char *filename, int *num_tokens_ret) {
    ...
    char line[1024]; 
    int linha=1;
    while(fgets(line,sizeof(line),file)) tokenizeLine(line,linha++);
    fclose(file);
...
int processarDeclaracao(TokenNode *token, char *tipo_atual, char *escopo_atual) {
    ...
    if (atual && ehOperador(atual->token) && strcmp(atual->token, "=") == 0) {
        ...
        atual = atual->prox;
        if (atual) strcpy(valor_atual, atual->token);
    ...
    // Caso seja declaração sem inicialização
        ...
        tabela_simbolos = adicionarSimbolo(
            tabela_simbolos, tipo_atual, nome_atual, "", escopo_atual
        );
...
int VerificaSintaxeEhValida(TokenNode *head) {
    ...
    while(current) {
        ...
        if(strcmp(token," ")==0){current=current->prox; continue;}
        if(!token || strlen(token)==0){current=current->prox; continue;}
        ...
        } else if(ehFuncao(token)) { strcpy(escopo_atual,token); }
        else if(ehVariavel(token)){
          //se o proximo token for um operador de atribuição , eu pego o valor e atribuo a variavel
          if (current->prox && ehOperador(current->prox->token) && strcmp(current->prox->token, "=") == 0) {
        ...
        else if(ehNumero(token) || ehString(token) || ehOperador(token) || ehMarcador(token)) {}
        else {
...
int verificarBalanceamento(TokenNode *token_list) {
    ...
    while (current) {
        ...
        // Fecha delimitadores
            ...
            if (isEmpty(pilha)) {
              printf("-={********************************************************}=-\n");
                printf("\t\tERRO na linha %d: delimitador de fechamento '%s' sem par!\n", linha, tok);
              printf("-={********************************************************}=-\n");
                return 0;
            }
            ...
            if ((aberto == '(' && tok[0] != ')') ||
                ...
                (aberto == '[' && tok[0] != ']')) {
              printf("-={********************************************************}=-\n");
                printf("\t\tERRO: delimitador '%c' aberto na linha %d foi fechado incorretamente com '%s' na linha %d\n",
